...
# Importa las clases y funciones necesarias de la
#  biblioteca Tkinter para crear la interfaz gráfica.
#import PIL.ImageGrab as ImageGrab: Importa el módulo
#ImageGrab de la biblioteca PIL para capturar y guardar imágenes.
linea_x = 0 #inicialiso pinsel
linea_y = 0
color = 'black' #color = 'black': Inicializa la variable color 
#con el valor 'black', que representa el color de dibujo predeterminado.

def linea_xy(event):#defino el metodo que va a crear la lia que se haga con line x,y
	global linea_x
	global linea_y
...
# Define una función llamada linea que se ejecuta cuando se arrastra el mouse sobre el lienzo (canvas). 
#Esta función dibuja una línea en el lienzo desde las coordenadas guardadas
#en las variables linea_x y linea_y hasta las coordenadas actuales del evento de arrastre.
def linea(event):
...
def mostrar_color(nueva_color):
	global color #variable global del metodo mostar color 
	color = nueva_color
